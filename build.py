...
def _hex_edge_neighbor(i: int, j: int, edge_idx: int) -> tuple[int, int]:
    ...
    Edges are labeled clockwise starting from the upper-right:
      0: top → top-right        (NE neighbor)
      1: top-right → bottom-right (E)
      2: bottom-right → bottom  (SE)
      3: bottom → bottom-left   (SW)
      4: bottom-left → top-left (W)
      5: top-left → top         (NW)

    For pointy-top tilings, NE/NW/SE/SW depend on row parity."""
    odd = (j % 2) != 0
...
def build_engines_panel_svg(
    ...
    for j in range(-j_max, j_max + 1):
        for i in range(-i_max, i_max + 1):
            ...
            # 3 internal Y-lines from center to top / BR / BL vertices (the
            # visible cube corner edges) — always drawn; never shared with
            # neighbors, so neither side has to skip them.
            y_arms = (
            ...
            if (i, j) in taken:
                # Engine tile: draw the full 6-edge outline + Y as a single
                # closed path so the dashed stroke runs continuously.
                outline = (
                    f"M {verts[0][0]:.2f} {verts[0][1]:.2f} "
                    + " ".join(f"L {x:.2f} {y:.2f}" for x, y in verts[1:])
                    + " Z"
                )
                taken_hex_paths.append(outline + y_arms)
            else:
                # Empty tile: build the outline edge-by-edge and OMIT any
                # edge shared with a taken neighbor — that edge is already
                # drawn (dashed) by the engine tile. Drawing the solid
                # version from this side would visually overwrite it.
                edge_segs: list[str] = []
                for e in range(6):
                    if _hex_edge_neighbor(i, j, e) in taken:
                        continue
                    v0 = verts[e]
                    v1 = verts[(e + 1) % 6]
                    edge_segs.append(
                ...
                hex_paths.append(" ".join(edge_segs) + " " + y_arms)
